...
#[test]
fn mvcc_gc_policy_identity_is_independent_of_backend_and_compiled_cc_configuration() {
    let state = TransactionState::default();
    let policy_identity = state.persistent_gc_policy_identity_for_test();
    let configurations = [
    ...
    assert_eq!(
        configurations
            .iter()
            .map(TransactionConfig::tmemory_backend)
            .collect::<Vec<_>>(),
        vec![
    ...
    for configuration in configurations {
        assert_eq!(
            configuration.concurrency_control(),
            ConcurrencyControl::default_for_build()
        );
        drop(TMemory::new(configuration, 1, Some(1)).unwrap());
        assert_eq!(
            state.persistent_gc_policy_identity_for_test(),
            policy_identity
        );
    }
}
...
#[test]
fn mvcc_gc_policy_shared_replacement_cannot_interleave_with_admitted_old_policy() {
    let runtime = TransactionRegionRuntime::default();
    ...
    let release_callback_rx = Arc::new(std::sync::Mutex::new(release_callback_rx));
    let old_hook_release = release_callback_rx.clone();
    let (old_policy, old_calls) = RecordingPersistentGc::with_hook(
        GcMvccMode::CurrentStateOnly,
        Arc::new(move |call, _mvcc, state, objects| {
            assert_eq!(call, MvccGcPolicyCall::Maintenance);
            callback_entered_tx.send(()).unwrap();
            old_hook_release.lock().unwrap().recv().unwrap();
            state
                .persistent_gc_maintenance_step_uncoordinated(
                    objects,
                    PersistentGcBudget::objects(1),
                )
                .map(|_| ())
    ...
    runtime
        .set_persistent_gc_policy_for_test(old_policy)
        .unwrap();
    let admission_epoch = runtime.persistent_gc_epoch().unwrap();

    let new_calls = Arc::new(std::sync::Mutex::new(Vec::new()));
    let new_policy = Arc::new(RecordingPersistentGc {
        mode: GcMvccMode::CurrentStateOnly,
        panic_on_mvcc_mode_query: false,
        calls: new_calls.clone(),
        hook: Some(Arc::new(|call, _mvcc, state, objects| {
            assert_eq!(call, MvccGcPolicyCall::Maintenance);
            state
                .persistent_gc_maintenance_step_uncoordinated(
                    objects,
    ...
    let mut objects = ObjectTable::default();
    let root = objects
        .allocate_persistent_struct_for_gc_ref(0x6c33, vec![ObjectValue::I32(1)])
        .unwrap();
    let mut state = TransactionState::default();
    state.set_shared_region_runtime(Some(runtime.clone()));
    state.install_recovered_persistent_roots([root]).unwrap();
    ...
    let replacement_error = runtime
        .set_persistent_gc_policy_for_test(new_policy.clone())
        .unwrap_err();
    assert_eq!(
        replacement_error.to_string(),
        "persistent GC is already active"
    );
    assert_eq!(
        runtime.persistent_gc_epoch().unwrap(),
        admission_epoch,
        "failed replacement must not advance the epoch"
    );
    release_callback_tx.send(()).unwrap();
    let (mut state, mut objects, result) = collector.join().unwrap();
    result.unwrap();
    assert_eq!(
        state.persistent_gc_state.as_ref().unwrap().shared_epoch(),
        Some(admission_epoch)
    );
    assert_eq!(
        *old_calls.lock().unwrap(),
        vec![MvccGcPolicyCall::Maintenance]
    );
    assert!(new_calls.lock().unwrap().is_empty());

    runtime
        .set_persistent_gc_policy_for_test(new_policy)
        .unwrap();
    let replacement_epoch = runtime.persistent_gc_epoch().unwrap();
    assert_eq!(replacement_epoch, admission_epoch + 1);
    state
        .persistent_gc_maintenance_step_for_test(&mut objects, PersistentGcBudget::objects(1))
        .unwrap();
...
#[test]
fn mvcc_gc_policy_shared_replacement_rejects_active_user_commit_without_mutation() {
    let runtime = TransactionRegionRuntime::default();
    let original_epoch = runtime.persistent_gc_epoch().unwrap();
    let user_commit = runtime.begin_user_transaction_region().unwrap();
    let (replacement, replacement_calls) = RecordingPersistentGc::new(GcMvccMode::CurrentStateOnly);

    let error = runtime
        .set_persistent_gc_policy_for_test(replacement.clone())
        .unwrap_err();

    assert_eq!(error.to_string(), "user transaction commit is active");
    assert_eq!(runtime.persistent_gc_epoch().unwrap(), original_epoch);
    drop(user_commit);
    runtime
        .set_persistent_gc_policy_for_test(replacement)
        .unwrap();
    let mut state = TransactionState::default();
    state.set_shared_region_runtime(Some(runtime));
    let mut objects = ObjectTable::default();
    state
        ...
        .unwrap();
    assert_eq!(
        *replacement_calls.lock().unwrap(),
        vec![MvccGcPolicyCall::MarkSweep]
    );
}
...
fn mvcc_gc_commit_six_domains(runtime: &MvccRuntime, objects: &mut ObjectTable) -> (u64, ObjectId) {
    ...
    let object_before = ObjectPayload::Struct(vec![ObjectValue::I32(1)]);
    let object_after = ObjectPayload::Struct(vec![ObjectValue::I32(2)]);
    let object = objects
        .allocate_persistent_struct_for_gc_ref(0x6c02, vec![ObjectValue::I32(1)])
        .unwrap();
    ...
    prepare
        .prepare_object(object, || Ok(Some(object_before)), object_after.clone())
        .unwrap();
    drop(prepare);
    objects
        .install_current_payload_snapshot(object, &object_after)
        .unwrap();
...
fn mvcc_gc_commit_object(
    ...
    let mut prepare = runtime.begin_prepare(commit).unwrap();
    prepare
        .prepare_object(object, || Ok(predecessor), value)
        .unwrap();
    drop(prepare);
    pending.publish().unwrap()
}
...
fn mvcc_gc_budget_is_bounded_and_round_robin_visits_all_six_tables_then_wraps() {
    ...
    let mut objects = ObjectTable::default();
    assert_eq!(mvcc_gc_commit_six_domains(&runtime, &mut objects).0, 1);
...
fn mvcc_gc_exact_barrier_rebase_exposes_objects_then_clears_cursors_and_advances_baseline() {
    ...
    assert_eq!(objects.is_persistent(object).unwrap(), true);
    assert_eq!(runtime.total_version_chain_count_for_test().unwrap(), 6);

    let validated = plan.validate_current_objects(&mut objects).unwrap();
    let metadata = runtime.complete_full_rebase(&barrier, validated).unwrap();
...
fn mvcc_gc_barrier_rebase_purges_residual_aborted_only_object_chain() {
    ...
    let validated = plan.validate_current_objects(&mut objects).unwrap();
    runtime.complete_full_rebase(&barrier, validated).unwrap();
    assert_eq!(runtime.object_chain_count_for_test().unwrap(), 0);
}
...
fn mvcc_gc_object_validation_rejects_payload_equal_volatile_slot() {
    ...
    let runtime = visibility.runtime();
    let mut objects = ObjectTable::default();
    let object = objects.allocate_struct(vec![ObjectValue::I32(7)]).unwrap();
    let payload = objects.current_payload_snapshot(object).unwrap().unwrap();
    assert_eq!(
        mvcc_gc_commit_object(runtime, object, Some(payload.clone()), payload),
        1
    );
    let barrier = visibility.begin_gc_barrier_for_test().unwrap();
    let plan = runtime.prepare_full_rebase(&barrier).unwrap();

    let error = plan.validate_current_objects(&mut objects).unwrap_err();

    assert_eq!(
        error.to_string(),
        format!("MVCC GC rebase expected a live persistent object slot: {object:?}")
    );
    assert_eq!(runtime.object_version_count_for_test(object).unwrap(), 2);
    assert_eq!(
        runtime
            .coordinator_metadata_for_test()
            .unwrap()
...
fn mvcc_gc_object_validation_rejects_payload_mismatch_without_mutation() {
    ...
    let mut objects = ObjectTable::default();
    let object = objects
        .allocate_persistent_struct_for_gc_ref(0x6c04, vec![ObjectValue::I32(8)])
        .unwrap();
    let predecessor = objects.current_payload_snapshot(object).unwrap();
    let expected = ObjectPayload::Struct(vec![ObjectValue::I32(9)]);
    assert_eq!(
        mvcc_gc_commit_object(runtime, object, predecessor, expected.clone()),
        1
    );
    let barrier = visibility.begin_gc_barrier_for_test().unwrap();
    let plan = runtime.prepare_full_rebase(&barrier).unwrap();
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "MVCC GC rebase current object payload does not match the newest committed version: \
             {object:?}"
        )
    );
    assert_eq!(runtime.object_version_count_for_test(object).unwrap(), 2);
    ...
    assert_eq!(
        runtime
            .read_object(snapshot.timestamp(), object, || {
                panic!("validation failure must retain the object version chain")
            })
            .unwrap(),
        Some(expected)
    );
...
fn mvcc_gc_object_validation_requires_aborted_only_new_object_to_be_absent() {
    ...
    let mut objects = ObjectTable::default();
    let object = objects
        .allocate_persistent_struct_for_gc_ref(0x6c05, vec![ObjectValue::I32(10)])
        .unwrap();
    let payload = objects.current_payload_snapshot(object).unwrap().unwrap();
    let (commit, mut pending) = runtime.begin_pending_commit().unwrap();
    let mut prepare = runtime.begin_prepare(commit.clone()).unwrap();
    prepare
        .prepare_object(object, || Ok(None), payload)
        .unwrap();
    drop(prepare);
    pending.abort().unwrap();
    let barrier = visibility.begin_gc_barrier_for_test().unwrap();
    ...
    assert_eq!(
        error.to_string(),
        format!("MVCC GC rebase expected the current object slot to be absent: {object:?}")
    );
    assert_eq!(runtime.object_version_count_for_test(object).unwrap(), 1);
    assert_eq!(
        runtime
...
fn mvcc_gc_rebase_rejects_validated_token_from_a_prior_barrier_epoch() {
    let visibility = SelectedTransactionVisibility::default();
    let runtime = visibility.runtime();
    let granule = mvcc_certification_granule(46);
    assert_eq!(mvcc_gc_commit_memory(runtime, granule, 1), 1);
    let mut objects = ObjectTable::default();
    let first_barrier = visibility.begin_gc_barrier_for_test().unwrap();
    let validated = runtime
        .prepare_full_rebase(&first_barrier)
        .unwrap()
        .validate_current_objects(&mut objects)
        .unwrap();
    drop(first_barrier);
    let second_barrier = visibility.begin_gc_barrier_for_test().unwrap();

    let error = runtime
        .complete_full_rebase(&second_barrier, validated)
        .unwrap_err();
    ...
    assert_eq!(runtime.granule_version_count_for_test(granule).unwrap(), 2);
    assert_eq!(
        runtime
            .coordinator_metadata_for_test()
            .unwrap()
    ...
    assert_eq!(
        runtime
            .read_memory(snapshot.timestamp(), granule, || {
                panic!("stale validation must not clear the version chain")
            })
            .unwrap(),
        vec![1]
    );
...
fn mvcc_gc_rebase_rejects_validated_token_from_another_coordinator() {
    ...
    let mut objects = ObjectTable::default();
    let first_barrier = first.begin_gc_barrier_for_test().unwrap();
    let validated = first
        .runtime()
        .prepare_full_rebase(&first_barrier)
        .unwrap()
        .validate_current_objects(&mut objects)
        .unwrap();
    let second_barrier = second.begin_gc_barrier_for_test().unwrap();
    ...
    drop(second_barrier);
    drop(first_barrier);
    second.begin_snapshot().unwrap().finish().unwrap();
}
...
fn mvcc_gc_rebase_revalidates_object_sidecars_after_current_object_validation() {
    ...
    let late_commit = Arc::new(CommitRecord::pending());
    let mut prepare = runtime.begin_prepare(late_commit.clone()).unwrap();
    prepare
        .prepare_object(
            object,
            || Ok(None),
        ...
        .unwrap();
    drop(prepare);
    late_commit.commit(1).unwrap();

    let error = runtime
        .complete_full_rebase(&barrier, validated)
        .unwrap_err();
...
fn mvcc_gc_rebase_rejects_second_validated_token_after_baseline_changes() {
    let visibility = SelectedTransactionVisibility::default();
    let runtime = visibility.runtime();
    let granule = mvcc_certification_granule(47);
    assert_eq!(mvcc_gc_commit_memory(runtime, granule, 1), 1);
    let mut objects = ObjectTable::default();
    let barrier = visibility.begin_gc_barrier_for_test().unwrap();
    let first = runtime
    ...
    let second = runtime
        ...
        .validate_current_objects(&mut objects)
        .unwrap();
    runtime.complete_full_rebase(&barrier, first).unwrap();

    let error = runtime.complete_full_rebase(&barrier, second).unwrap_err();
    ...
    transaction_test_module(
        ...
        r#"
            (module
              ...
              (tmemory 1)
              (tfunc (export "init") (param i32 i32 i32)
                (i32.tstore (i32.const 0) (local.get 0))
                (i32.tstore (i32.const 64) (local.get 1))
                (i32.tstore (i32.const 128) (local.get 2)))
              (tfunc (export "blind_a") (param i32)
                (i32.tstore (i32.const 0) (local.get 0))
                (call $sync))
              (tfunc (export "increment_a")
                (local $old i32)
                (local.set $old (i32.tload (i32.const 0)))
                (i32.tstore
                  (i32.const 0)
                  (i32.add (local.get $old) (i32.const 1)))
                (call $sync))
              (tfunc (export "read_a_write_b") (param i32)
                (drop (i32.tload (i32.const 0)))
                (i32.tstore (i32.const 64) (local.get 0))
                (call $sync))
              (tfunc (export "write_a") (param i32)
                (call $sync)
                (i32.tstore (i32.const 0) (local.get 0)))
              (tfunc (export "write_b") (param i32)
                (call $sync)
                (i32.tstore (i32.const 64) (local.get 0)))
              (tfunc (export "write_skew_a")
                (local $a i32)
                (local $b i32)
                (local.set $a (i32.tload (i32.const 0)))
                (local.set $b (i32.tload (i32.const 64)))
                (if (i32.and
                      (i32.eq (local.get $a) (i32.const 1))
                      (i32.eq (local.get $b) (i32.const 1)))
                  (then (i32.tstore (i32.const 0) (i32.const 0))))
                (call $sync))
              (tfunc (export "write_skew_b")
                ...
                (if (i32.and
                      (i32.eq (local.get $a) (i32.const 1))
                      (i32.eq (local.get $b) (i32.const 1)))
                  (then (i32.tstore (i32.const 64) (i32.const 0))))
                (call $sync))
              (tfunc (export "read_a_twice") (result i32 i32)
                (local $first i32)
...
fn mvcc_serializable_assert_runtime_quiescent(runtime: &TransactionRegionRuntime) {
    ...
    let (active, begun, finished, dropped) =
        visibility.snapshot_lifecycle_counts_for_test().unwrap();
    assert_eq!(active, 0);
    assert_eq!(begun, finished + dropped);
    let (registered, published, aborted) = visibility
        .runtime()
        .commit_lifecycle_counts_for_test()
        .unwrap();
    assert_eq!(registered, published + aborted);
    drop(visibility.begin_gc_barrier_for_test().unwrap());
    drop(runtime.begin_persistent_gc_for_test().unwrap());
}
...
fn mvcc_serializable_write_skew_aborts_one_writer() {
    ...
    let (mut init_store, init_instance) =
        mvcc_serializable_shared_memory_store(&engine, &module, &runtime, Arc::new(|| Ok(())));
    init_instance
        .get_typed_func::<(i32, i32, i32), ()>(&mut init_store, "init")
        .unwrap()
        .call(&mut init_store, (1, 1, 0))
        .unwrap();
    drop(init_store);
    ...
    mvcc_serializable_assert_one_commit_one_conflict(&outcomes);
    ...
    let (a, b, _) = read_instance
        ...
        .call(&mut read_store, ())
        .unwrap();
    assert!(a + b >= 1, "write skew violated A + B >= 1: A={a}, B={b}");
...
fn mvcc_serializable_blind_write_write_aborts_one_writer() {
    ...
    let (mut read_store, read_instance) =
        mvcc_serializable_shared_memory_store(&engine, &module, &runtime, Arc::new(|| Ok(())));
    let (a, _, _) = read_instance
        .get_typed_func::<(), (i32, i32, i32)>(&mut read_store, "read")
        .unwrap()
        .call(&mut read_store, ())
        .unwrap();
    assert!(matches!(a, 2 | 3));
    mvcc_serializable_assert_runtime_quiescent(&runtime);
    clear_current_thread_transaction_for_test();
}
...
fn mvcc_serializable_changed_dependency_aborts_transaction_with_disjoint_write() {
    ...
    let dependency_ready = Arc::new(MvccCommitTestHook::new(1));
    let thread_ready = dependency_ready.clone();
    let (mut dependent_store, dependent_instance) = mvcc_serializable_shared_memory_store(
        &engine,
        &module,
        &runtime,
        Arc::new(move || thread_ready.wait()),
    ...
    let dependent_writer = std::thread::spawn(move || {
        ...
        outcome_tx
            .send(
                dependent
                    .call(&mut dependent_store, 9)
                    .map_err(|error| format!("{error:#}")),
            )
            .unwrap();
    });
    if !dependency_ready.wait_until_reached(Duration::from_secs(5)) {
        dependency_ready.release();
...
fn mvcc_serializable_read_only_snapshot_stays_stable_during_writer() {
    ...
    let reader_paused = Arc::new(MvccCommitTestHook::new(1));
    let reader_gate = reader_paused.clone();
    let (mut reader_store, reader_instance) = mvcc_serializable_shared_memory_store(
        &engine,
        &module,
        &runtime,
        Arc::new(move || reader_gate.wait()),
    );
    ...
    let reader = std::thread::spawn(move || {
        ...
        outcome_tx
            .send(
                read_twice
                    .call(&mut reader_store, ())
                    .map_err(|error| format!("{error:#}")),
            )
            .unwrap();
    });
    if !reader_paused.wait_until_reached(Duration::from_secs(5)) {
        reader_paused.release();
        panic!("read-only transaction did not reach its deterministic read gate");
    }
    let certification_before = runtime.mvcc_certification_counts_for_test().unwrap().0;
    let commits_before = runtime
        .visibility_for_test()
        .runtime()
        .commit_lifecycle_counts_for_test()
        .unwrap()
    ...
    assert_eq!(reads, (1, 1));
    assert_eq!(
        runtime.mvcc_certification_counts_for_test().unwrap().0,
        certification_before + 1
    );
    assert_eq!(
        runtime
            .visibility_for_test()
...
fn mvcc_serializable_disjoint_writers_overlap_terminal_publication() {
    ...
    if !inside_install.wait_until_reached(Duration::from_secs(5)) {
        inside_install.release();
        panic!("disjoint writers did not overlap inside terminal installation");
    }
    assert_eq!(
        runtime.mvcc_certification_counts_for_test().unwrap(),
        (2, 2)
...
fn mvcc_serializable_reader_never_mixes_pending_commit_domains() {
    ...
    let runtime = TransactionRegionRuntime::new_for_test();
    let visibility = runtime.visibility_for_test();
    let before_publish = Arc::new(MvccCommitTestHook::new(1));
    let after_publish = Arc::new(MvccCommitTestHook::new(1));
    visibility
        .runtime()
        .set_commit_hooks_for_test(
            Some(before_publish.clone()),
            Some(after_publish.clone()),
            None,
        )
        .unwrap();
    ...
    if !before_publish.wait_until_reached(Duration::from_secs(5)) {
        before_publish.release();
        after_publish.release();
        panic!("writer did not reach the pre-publication gate");
    }
    assert!(matches!(
        visibility
            .runtime()
    ...
    let mut old_reader = TransactionState::default();
    old_reader.begin_with_region_runtime(&runtime).unwrap();
    let old_context = old_reader.active_visibility_read_context().unwrap();
    let old_memory = old_context
        .read_memory(memory, || panic!("prepared memory chain must exist"))
        .unwrap();
    assert_eq!(old_memory[..4], 0_i32.to_le_bytes());

    before_publish.release();
    if !after_publish.wait_until_reached(Duration::from_secs(5)) {
        after_publish.release();
        panic!("writer did not reach the post-publication gate");
    }
    assert!(matches!(
        visibility
            .runtime()
    ...
    assert_eq!(
        old_context
            .read_global(global, || panic!("prepared global chain must exist"))
            .unwrap(),
        GlobalSnapshot::I32(3)
    );
    assert_eq!(
    ...
    assert_eq!(
        old_context
            .read_table(table, || panic!("prepared table chain must exist"))
            .unwrap()
            .elements(),
        &[TableElementSnapshot::FuncRef(0)]
    );
    ...
    let mut fresh_reader = TransactionState::default();
    fresh_reader.begin_with_region_runtime(&runtime).unwrap();
    let fresh_context = fresh_reader.active_visibility_read_context().unwrap();
    assert_eq!(
        fresh_context
            .read_memory(memory, || panic!("committed memory chain must exist"))
            .unwrap()[..4],
        7_i32.to_le_bytes()
    );
    assert_eq!(
        fresh_context
            .read_global(global, || panic!("committed global chain must exist"))
            .unwrap(),
        GlobalSnapshot::I32(9)
    );
    assert!(matches!(
        fresh_context
            .read_table(table, || panic!("committed table chain must exist"))
            .unwrap()
            .elements(),
        [TableElementSnapshot::FuncRef(value)] if *value != 0
    ));
    fresh_reader.abort().unwrap();
    mvcc_serializable_assert_runtime_quiescent(&runtime);
    clear_current_thread_transaction_for_test();
}
...
fn mvcc_serializable_model_value(bytes: &[u8]) -> Result<i64> {
    Ok(i64::from_le_bytes(
        bytes
            .get(..8)
            .context("model memory payload is shorter than i64")?
            .try_into()
            .unwrap(),
    ))
}
...
fn mvcc_serializable_compare_model_schedule(
    ...
    let transaction_count = operations
        ...
        .map(|operation| match operation {
            ...
            | ModelOperation::Read { transaction, .. }
            | ModelOperation::Write { transaction, .. }
            | ModelOperation::Commit { transaction } => *transaction,
        })
        .max()
        .context("generated schedule has no transactions")?
        + 1;
    let granules = operations
        .iter()
        .filter_map(|operation| match operation {
            ModelOperation::Read { granule, .. } | ModelOperation::Write { granule, .. } => {
                Some(*granule)
            }
            ModelOperation::Begin { .. } | ModelOperation::Commit { .. } => None,
        })
        .collect::<BTreeSet<_>>();
    ensure!((2..=4).contains(&transaction_count));
    ensure!((2..=4).contains(&granules.len()));
    let baseline = granules
        .iter()
        .enumerate()
        .map(|(index, granule)| (*granule, i64::try_from(index).unwrap() + 10))
        .collect::<BTreeMap<_, _>>();

    let runtime = TransactionRegionRuntime::new_for_test();
    let visibility = runtime.visibility_for_test();
    let mvcc = visibility.runtime().clone();
    let mut model = SerializableMvccModel::with_values(
        baseline.iter().map(|(granule, value)| (*granule, *value)),
    );
    let mut model_transactions = (0..transaction_count)
        .map(|_| None)
        .collect::<Vec<Option<ModelTransaction>>>();
    ...
    for (operation_order, operation) in operations.iter().enumerate() {
        match *operation {
            ModelOperation::Begin { transaction } => {
                ...
                ensure!(real_transactions[transaction].is_none());
                let model_transaction = model.begin();
                let registration = visibility.begin_snapshot()?;
                let snapshot = SelectedTransactionVisibility::snapshot_timestamp(&registration);
                ensure!(model_transaction.snapshot() == snapshot);
                model_transactions[transaction] = Some(model_transaction);
            ...
            ModelOperation::Read {
                ...
                let model_transaction = model_transactions[transaction]
                    .as_mut()
                    .context("model read transaction is not active")?;
                let model_value = model.read(model_transaction, granule);
                ...
                let real_value = match real_transaction.writes.get(&granule).copied() {
                    ...
                    None => mvcc_serializable_model_value(&mvcc.read_memory(
                        real_transaction.snapshot,
                        granule,
                        || Ok(mvcc_serializable_model_bytes(baseline[&granule])),
                    )?)?,
                };
                ensure!(
            ...
            ModelOperation::Commit { transaction } => {
                let model_transaction = model_transactions[transaction]
                    .take()
                    .context("model commit transaction is not active")?;
                let expected_reads = model_transaction.reads().clone();
                let expected_writes = model_transaction.writes().clone();
                let model_outcome = model.commit(model_transaction);
                let real_transaction = real_transactions[transaction]
                    .take()
                    .context("real commit transaction is not active")?;
                ensure!(real_transaction.reads == expected_reads);
                ensure!(real_transaction.writes == expected_writes);

                let real_outcome = if real_transaction.writes.is_empty() {
                    real_transaction.registration.finish()?;
                    ModelCommitOutcome::Committed(None)
                } else {
                    let writes = real_transaction
                        .writes
                        .keys()
                        .copied()
                        .collect::<BTreeSet<_>>();
                    match runtime.acquire_mvcc_certification(
                        real_transaction.id,
                        &real_transaction.reads,
                        &writes,
                        real_transaction.snapshot,
                        &mvcc,
                    ) {
                        Ok(permit) => {
                            let (commit, mut pending) = mvcc.begin_pending_commit()?;
                            let mut prepare = mvcc.begin_prepare(commit)?;
                            for (granule, value) in &real_transaction.writes {
                                prepare.prepare_memory(
                                    *granule,
                                    || {
                                        Ok(mvcc_serializable_model_bytes(
                                            baseline.get(granule).copied().unwrap_or_default(),
                                        ))
                                    },
                                    mvcc_serializable_model_bytes(*value),
                                )?;
                            }
                            drop(prepare);
                            let timestamp = pending.publish()?;
                            drop(permit);
                            real_transaction.registration.finish()?;
    ...
    ensure!(model.visible_timestamp() == real_visible);
    final_snapshot.finish()?;
    let mut real_final = BTreeMap::new();
    for granule in granules.iter().copied() {
        let value =
            mvcc_serializable_model_value(&mvcc.read_memory(real_visible, granule, || {
                Ok(mvcc_serializable_model_bytes(baseline[&granule]))
            })?)?;
        ensure!(model.visible_value(granule) == value);
    ...
    ensure!(begun == finished + dropped);
    ensure!(runtime.mvcc_certification_counts_for_test()?.1 == 0);
    let (registered, published, aborted) = mvcc.commit_lifecycle_counts_for_test()?;
...
fn mvcc_visibility_repeated_tmemory_loads_remain_stable() {
    ...
    let module = transaction_test_module(
        ...
        r#"
            (module
              ...
              (tmemory $m (export "memory") 1)
              (func (export "read_twice") (result i32 i32)
                (local $first i32)
                (local.set $first (i32.tload $m (i32.const 0)))
                (call $advance)
                (local.get $first)
                (i32.tload $m (i32.const 0))))
        "#,
    ...
    linker
        .func_wrap(
            ...
            move |caller: crate::Caller<'_, ()>| -> Result<()> {
                ...
                let mut baseline = vec![0; TMEMORY_GRANULE_SIZE];
                baseline[..4].copy_from_slice(&11_i32.to_le_bytes());
                let mut current = baseline.clone();
                current[..4].copy_from_slice(&22_i32.to_le_bytes());
                let commit = Arc::new(CommitRecord::pending());
                let mut prepare = host_visibility.runtime().begin_prepare(commit.clone())?;
                prepare.prepare_memory(granule, || Ok(baseline), current.clone())?;
                drop(prepare);
                commit.commit(1)?;
    ...
    store
        ...
        .unwrap();
    let read_twice = instance
        .get_typed_func::<(), (i32, i32)>(&mut store, "read_twice")
        .unwrap();
    ...
    store
        ...
        .begin_with_region_runtime(&runtime)
        .unwrap();
    assert_eq!(read_twice.call(&mut store, ()).unwrap(), (11, 11));
...
fn mvcc_visibility_partial_and_cross_granule_tmemory_stores_use_one_snapshot() {
    ...
    let module = transaction_test_module(
        ...
        r#"
            (module
              (import "host" "advance" (func $advance))
              (tmemory $m 1)
              (func (export "store_and_read") (result i64)
                (call $advance)
                (i32.tstore $m (i32.const 62) (i32.const 0x44332211))
                (i64.tload $m (i32.const 60))))
        "#,
    );
    let runtime = TransactionRegionRuntime::new_for_test();
    let visibility = runtime.visibility_for_test();
    ...
    linker
        .func_wrap(
            ...
            move |caller: crate::Caller<'_, ()>| -> Result<()> {
                let owner = host_instance_id.load(Ordering::SeqCst);
                let mut old0 = vec![0xa0; TMEMORY_GRANULE_SIZE];
                let mut old1 = vec![0xa1; TMEMORY_GRANULE_SIZE];
                old0[60..].copy_from_slice(&[0x10, 0x11, 0x12, 0x13]);
                old1[..4].copy_from_slice(&[0x14, 0x15, 0x16, 0x17]);
                let mut new0 = vec![0xb0; TMEMORY_GRANULE_SIZE];
                let mut new1 = vec![0xb1; TMEMORY_GRANULE_SIZE];
                new0[60..].copy_from_slice(&[0x20, 0x21, 0x22, 0x23]);
                new1[..4].copy_from_slice(&[0x24, 0x25, 0x26, 0x27]);
                let commit = Arc::new(CommitRecord::pending());
                let mut prepare = host_visibility.runtime().begin_prepare(commit.clone())?;
                prepare.prepare_memory(
                    GranuleId::TMemory {
    ...
    instance_id.store(instance.id().as_u32(), Ordering::SeqCst);
    {
        ...
        tmemory
            .commit_range(TMEMORY_GRANULE_SIZE, &[0x14, 0x15, 0x16, 0x17])
            .unwrap();
    }
    let store_and_read = instance
        .get_typed_func::<(), i64>(&mut store, "store_and_read")
        .unwrap();
    ...
    store
        ...
        .unwrap();
    assert_eq!(
        store_and_read.call(&mut store, ()).unwrap() as u64,
        u64::from_le_bytes([0x10, 0x11, 0x11, 0x22, 0x33, 0x44, 0x16, 0x17])
    );
    store.transaction_state_mut().abort().unwrap();
}
...
fn mvcc_visibility_tmemory_size_controls_bounds_and_staged_growth() {
    ...
    let module = transaction_test_module(
        ...
        r#"
            (module
              (import "host" "advance" (func $advance))
              (tmemory $m 1 3)
              (func (export "snapshot_oob") (result i32)
                (call $advance)
                (i32.tload $m (i32.const 65536)))
              (func (export "size_and_grow") (result i32 i32)
                (call $advance)
                (tmemory.size $m)
                (tmemory.grow $m (i32.const 1))))
        "#,
    );
    let runtime = TransactionRegionRuntime::new_for_test();
    let visibility = runtime.visibility_for_test();
    ...
    let host_advanced = Arc::clone(&advanced);
    let mut store = crate::Store::new(&engine, ());
    store.set_transaction_region_runtime_for_test(runtime.clone());
    let mut linker = crate::Linker::new(&engine);
    linker
        .func_wrap(
            ...
            "advance",
            move |caller: crate::Caller<'_, ()>| -> Result<()> {
                if host_advanced.swap(true, Ordering::SeqCst) {
                    return Ok(());
                }
                let owner = host_instance_id.load(Ordering::SeqCst);
                let commit = Arc::new(CommitRecord::pending());
    ...
    let instance = linker.instantiate(&mut store, &module).unwrap();
    instance_id.store(instance.id().as_u32(), Ordering::SeqCst);
    let snapshot_oob = instance
        .get_typed_func::<(), i32>(&mut store, "snapshot_oob")
        .unwrap();
    let size_and_grow = instance
        .get_typed_func::<(), (i32, i32)>(&mut store, "size_and_grow")
        .unwrap();
    ...
    store
        ...
        .begin_with_region_runtime(&runtime)
        .unwrap();
    assert!(snapshot_oob.call(&mut store, ()).is_err());
    if store.transaction_state().active_transaction().is_some() {
        store.transaction_state_mut().abort().unwrap();
    }
    store
        .transaction_state_mut()
        .begin_with_region_runtime(&runtime)
        .unwrap();
    assert_eq!(size_and_grow.call(&mut store, ()).unwrap(), (1, 1));
    assert_eq!(
        store
            .transaction_state()
            .staged_memory_size_owned(Some(instance.id()), 0),
        Some(2)
    );
    store.transaction_state_mut().abort().unwrap();
}
...
fn mvcc_visibility_tglobal_reads_snapshot_and_staged_value_first() {
    ...
    let module = transaction_test_module(
        ...
        r#"
            (module
              (import "host" "advance" (func $advance))
              (tglobal $g (export "global") (mut i32) (i32.const 11))
              (func (export "read_then_stage") (result i32 i32 i32)
                (local $first i32)
                (local.set $first (tglobal.get $g))
                (call $advance)
                (local.get $first)
                (tglobal.get $g)
                (tglobal.set $g (i32.const 33))
                (tglobal.get $g)))
        "#,
    );
    let runtime = TransactionRegionRuntime::new_for_test();
    ...
    linker
        .func_wrap(
            ...
            move |mut caller: crate::Caller<'_, ()>| -> Result<()> {
                ...
                let commit = Arc::new(CommitRecord::pending());
                let mut prepare = host_visibility.runtime().begin_prepare(commit.clone())?;
                prepare.prepare_global(
                    GranuleId::TGlobal {
                        instance: Some(owner),
                        global_index: 0,
                    },
                    || Ok(GlobalSnapshot::I32(11)),
                    GlobalSnapshot::I32(22),
                )?;
                drop(prepare);
                commit.commit(1)?;
                caller
                    ...
                    .context("missing exported tglobal")?
                    .set(&mut caller, crate::Val::I32(22))?;
                Ok(())
            },
        )
        .unwrap();
    let instance = linker.instantiate(&mut store, &module).unwrap();
    instance_id.store(instance.id().as_u32(), Ordering::SeqCst);
    let read_then_stage = instance
        .get_typed_func::<(), (i32, i32, i32)>(&mut store, "read_then_stage")
        .unwrap();
    ...
    store
        ...
        .begin_with_region_runtime(&runtime)
        .unwrap();
    assert_eq!(read_then_stage.call(&mut store, ()).unwrap(), (11, 11, 33));
    store.transaction_state_mut().abort().unwrap();
}
...
fn mvcc_visibility_tglobal_reference_snapshot_is_stable() {
    ...
    let module = transaction_test_module(
        ...
        r#"
            (module
              ...
              (tglobal $g (export "global") (mut funcref) (ref.null func))
              (func (export "is_null_after_advance") (result i32)
                (call $advance)
                (ref.is_null (tglobal.get $g))))
        "#,
    );
    let runtime = TransactionRegionRuntime::new_for_test();
    let visibility = runtime.visibility_for_test();
        ...
        .func_wrap(
            ...
            move |mut caller: crate::Caller<'_, ()>| -> Result<()> {
                ...
                prepare.prepare_global(
                    ...
                    || Ok(GlobalSnapshot::FuncRef(0)),
                    GlobalSnapshot::FuncRef(1),
                )?;
                drop(prepare);
                commit.commit(1)?;
                let target = caller
                ...
                caller
                    ...
                    .and_then(|export| export.into_global())
                    .context("missing exported tglobal")?
                    .set(&mut caller, crate::Val::FuncRef(Some(target)))?;
                Ok(())
            },
        )
        .unwrap();
    let instance = linker.instantiate(&mut store, &module).unwrap();
    instance_id.store(instance.id().as_u32(), Ordering::SeqCst);
    let is_null_after_advance = instance
        .get_typed_func::<(), i32>(&mut store, "is_null_after_advance")
        .unwrap();
    ...
    store
        ...
        .begin_with_region_runtime(&runtime)
        .unwrap();
    assert_eq!(is_null_after_advance.call(&mut store, ()).unwrap(), 1);
    store.transaction_state_mut().abort().unwrap();
}
...
fn mvcc_visibility_ttable_reads_complete_granule_and_snapshot_size_for_growth() {
    ...
    let module = transaction_test_module(
        ...
        r#"
            (module
              ...
              (func $target)
              (export "target" (func $target))
              (ttable $t (export "table") 17 20 funcref)
              (func (export "read_and_grow") (result i32 i32 i32 i32)
                (call $advance)
                (ttable.size $t)
                (ref.is_null (ttable.get $t (i32.const 16)))
                (ttable.grow $t (ref.null func) (i32.const 1))
        ...
        .func_wrap(
            ...
            move |mut caller: crate::Caller<'_, ()>| -> Result<()> {
                ...
                prepare.prepare_table(
                    ...
                    || TableGranuleSnapshot::new(vec![TableElementSnapshot::FuncRef(0)]),
                    TableGranuleSnapshot::new(vec![
                        TableElementSnapshot::FuncRef(1),
                        TableElementSnapshot::FuncRef(0),
                    ])?,
                )?;
                drop(prepare);
                ...
                table.grow(&mut caller, 1, crate::Ref::Func(None))?;
                table.set(&mut caller, 16, crate::Ref::Func(Some(target)))?;
                Ok(())
            },
        )
        .unwrap();
    let instance = linker.instantiate(&mut store, &module).unwrap();
    instance_id.store(instance.id().as_u32(), Ordering::SeqCst);
    let read_and_grow = instance
        .get_typed_func::<(), (i32, i32, i32, i32)>(&mut store, "read_and_grow")
        .unwrap();
    ...
    store
        ...
        .begin_with_region_runtime(&runtime)
        .unwrap();
    assert_eq!(read_and_grow.call(&mut store, ()).unwrap(), (17, 1, 17, 1));
    store.transaction_state_mut().abort().unwrap();
}
...
fn mvcc_visibility_ttable_staged_growth_keeps_snapshot_granule_truncation() {
    ...
    let module = transaction_test_module(
        ...
        r#"
            (module
              (ttable $t 17 18 funcref)
              (func (export "grow_then_read_old") (result i32)
                (drop (ttable.grow $t (ref.null func) (i32.const 1)))
                (ref.is_null (ttable.get $t (i32.const 16)))))
        "#,
    );
    let runtime = TransactionRegionRuntime::new_for_test();
    ...
    store.set_transaction_region_runtime_for_test(runtime.clone());
    let instance = crate::Instance::new(&mut store, &module, &[]).unwrap();
    let grow_then_read_old = instance
        .get_typed_func::<(), i32>(&mut store, "grow_then_read_old")
        .unwrap();
    ...
    store
        ...
        .begin_with_region_runtime(&runtime)
        .unwrap();
    assert_eq!(grow_then_read_old.call(&mut store, ()).unwrap(), 1);
    store.transaction_state_mut().abort().unwrap();
}
...
fn mvcc_visibility_ttable_fill_copy_and_ranges_record_data_and_size_granules() {
    ...
    let host_recorded = Arc::clone(&recorded);
    let mut store = crate::Store::new(&engine, ());
    store.set_transaction_region_runtime_for_test(runtime);
    let mut linker = crate::Linker::new(&engine);
    linker
    ...
    let instance = linker.instantiate(&mut store, &module).unwrap();
    instance_id.store(instance.id().as_u32(), Ordering::SeqCst);
    let operate = instance
        .get_typed_func::<(), ()>(&mut store, "operate")
        .unwrap();
...
fn file_backed_mixed_commit_recovers_tmemory_and_reuses_committed_linear_undo_chunk() {
    ...
    recovered_objects
        .rebuild_from_recovery_with_source_for_test(
            &recovered_region.type_layouts,
            &object_winners,
            recovered_mapped_source_for_test(&recovered_region),
        )
        .unwrap();
    assert_eq!(
...
mod file_backed_object_layout_recovery {
    ...
    fn missing_layout_metadata_rejects_recovery() {
        ...
        let err = rebuilt
            .rebuild_reachable_from_recovery_with_source_for_test(
                ...
                &object_winners,
                &[41],
                recovered_mapped_source_for_test(&recovered),
            )
            .unwrap_err();
    ...
    fn recovered_object_table_installs_mapped_records_without_copying_record_bytes() -> Result<()> {
        ...
        assert_eq!(
            ...
            0,
            "mapped recovery rebuild must not materialize recovered object payloads through decode_record"
        );
        assert_eq!(
        ...
        assert!(
            objects.current_record_is_persistent_mapped_for_test(ObjectId { object_index: 41 })?
        );
        Ok(())
    }
    ...
    fn file_backed_persistent_gc_evacuates_mixed_chunk_and_retires_old_chunk() {
        ...
        let report = rebuilt
            .rebuild_reachable_from_recovery_with_source_for_test(
                ...
                recovered_mapped_source_for_test(&before_recovered),
            )
            .unwrap();
        assert_eq!(report.mark.reachable, object_set([live]));
        assert_eq!(report.mark.unreachable_persistent, object_set([dead]));
...
mod model_mixed_participants {
    ...
    fn run_mixed_model_case(case: &MixedModelCase) -> Result<ActualMixedWorld> {
        let outcome = (|| -> Result<ActualMixedWorld> {
            ...
            let mut recovered_objects = ObjectTable::default();
            recovered_objects.rebuild_from_recovery_with_source_for_test(
                &recovered_region.type_layouts,
                &object_winners,
                recovered_mapped_source_for_test(&recovered_region),
            )?;
...
mod model_object_rebuild {
    ...
    fn rebuild_object_table_from_winners(
        ...
        source: alloc::sync::Arc<dyn crate::runtime::vm::block_region::MappedRegionSource>,
    ) -> Result<ObjectTable> {
        let mut objects = ObjectTable::default();
        objects.rebuild_from_recovery_with_source_for_test(
    ...
    fn recover_object_history(history: &[ModelObjectRecord]) -> Result<RecoveredObjectHistory> {
        ...
        let object_winners = recovered_region.committed_object_winners()?;
        let mut rebuilt = ObjectTable::default();
        rebuilt.rebuild_from_recovery_with_source_for_test(
            &recovered_region.type_layouts,
            &object_winners,
            recovered_mapped_source_for_test(&recovered_region),
        )?;
    ...
    fn corrupt_payload_header_winner(
        ...
        recovered_object_winner_for_test(source, 7, 3, ObjectKind::Struct as u16, 207, record_bytes)
    }
    ...
    fn model_object_rebuild_rejects_corrupt_payload_header_mismatch() {
        ...
        let err = rebuild_object_table_from_winners(
            &recovered_type_layouts,
            &[corrupt_payload_header_winner(&fixture_source)],
    ...
    #[test]
    fn model_object_mapped_rebuild_rejects_corrupt_payload_header_mismatch() {
        let mut recovered_type_layouts = TypeLayoutRegistry::default();
        recovered_type_layouts
            .insert(PersistentTypeLayout::Struct {
                id: type_layout::TypeLayoutId::new(207).unwrap(),
...
fn transaction_local_commit_persists_only_root_reachable_graph() {
    ...
    assert_eq!(
        state.known_object_id_for_transaction_ref_handle(&objects, child_handle),
        Some(promoted_child)
    );
    assert_eq!(
        state.known_object_id_for_transaction_ref_handle(&objects, root_handle),
        Some(promoted_root)
    );
    assert_eq!(
        state.known_object_id_for_transaction_ref_handle(&objects, unreachable_handle),
        None
    );
    assert!(
        objects
            .known_object_id_for_transaction_ref_handle(unreachable_handle)
            .is_none()
    );
...
#[test]
fn transaction_local_object_promotion_rebinds_handle_to_promoted_object() {
    let mut objects = ObjectTable::default();
    let mut state = TransactionState::default();
...
fn transaction_local_object_promotion_preserves_cycles() {
    ...
    let promoted_left = objects
        .object_id_for_transaction_ref_handle(left_handle)
        .unwrap();
    let promoted_right = objects
...
fn object_index_rebuilds_from_recovered_object_winners() {
    ...
    let (winners, recovered_source) =
        copy_recovered_winners_from_view_into_synthetic_source_for_test(
            &region.view(),
            &recovered.committed_object_winners().unwrap(),
        );
    let mut objects = ObjectTable::default();
...
fn recovered_object_winner_with_location_for_test(
    ...
    let data_record_offset = register_recovered_object_record_in_synthetic_source_for_test(
        source,
        object_id,
        version,
        kind,
...
fn recovered_record_location_for_test(
    ...
    let durable_record_len = winner.durable_data_record_len().unwrap();
    object_gc::PersistentRecoveredRecordLocation {
        object_id: ObjectId {
            object_index: winner.object_id,
        },
...
fn recovered_mapped_source_for_test(
    recovered: &crate::runtime::vm::RecoveredRegion,
) -> alloc::sync::Arc<dyn crate::runtime::vm::block_region::MappedRegionSource> {
    recovered
        .cloned_mapped_region_source()
        .expect("recovered region should expose a mapped region source")
}

fn synthetic_recovered_mapped_source_for_test() -> (
    crate::runtime::vm::block_region::SyntheticRecoveredWinnerSourceHandle,
    alloc::sync::Arc<dyn crate::runtime::vm::block_region::MappedRegionSource>,
) {
    let source = crate::runtime::vm::block_region::new_synthetic_recovered_winner_source_for_test();
    let mapped_source =
        crate::runtime::vm::block_region::synthetic_recovered_winner_source_for_test(&source);
    (source, mapped_source)
}

fn register_recovered_object_record_in_synthetic_source_for_test(
    source: &crate::runtime::vm::block_region::SyntheticRecoveredWinnerSourceHandle,
    object_id: u64,
    version: u32,
    kind: u16,
    ...
    let domain = match kind {
        value if value == ObjectKind::Struct as u16 => {
            crate::runtime::vm::PackedGranuleDomain::TStruct
        }
        value if value == ObjectKind::Array as u16 => {
            crate::runtime::vm::PackedGranuleDomain::TArray
        }
        _ => unreachable!("unsupported synthetic recovered object kind {kind}"),
    };
    let data_record = TMemory::encode_publication_data_record(
        crate::runtime::vm::pack_object_granule_id(domain, object_id).unwrap(),
        version,
        domain as u16,
        type_layout_id,
        record_bytes,
...
fn copy_recovered_winners_from_view_into_synthetic_source_for_test(
    region: &crate::runtime::vm::block_region::BlockRegionBackendView<'_>,
    winners: &[crate::runtime::vm::RecoveredObjectWinner],
) -> (
    Vec<crate::runtime::vm::RecoveredObjectWinner>,
    alloc::sync::Arc<dyn crate::runtime::vm::block_region::MappedRegionSource>,
) {
    let (source, mapped_source) = synthetic_recovered_mapped_source_for_test();
    let winners = winners
        .iter()
        .cloned()
        .map(|mut winner| {
            ...
            winner
                .with_view_record_bytes(region, &mut |bytes| {
                    record_bytes.extend_from_slice(bytes);
                    Ok(())
                })
                .unwrap();
            winner.data_record_offset =
                register_recovered_object_record_in_synthetic_source_for_test(
                    &source,
                    winner.object_id,
                    winner.version,
                    winner.kind,
                    winner.type_layout_id,
                    &record_bytes,
                );
            winner
        })
        .collect();
    (winners, mapped_source)
}

fn synthetic_recovered_winners_for_test(
    build: impl FnOnce(
        &crate::runtime::vm::block_region::SyntheticRecoveredWinnerSourceHandle,
    ) -> Vec<crate::runtime::vm::RecoveredObjectWinner>,
) -> (
...
fn object_recovery_reference_graph_uses_layout_metadata_for_persistent_objects() {
    ...
    let first = ObjectId { object_index: 8 };
    let second = ObjectId { object_index: 9 };
    let (winners, recovered_source) = synthetic_recovered_winners_for_test(|source| {
        vec![recovered_object_winner_for_test(
            source,
            41,
            3,
            ObjectKind::Struct as u16,
